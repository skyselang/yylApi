...
CREATE TABLE `ya_interface`  (
  ...
  `response_data` text CHARACTER SET utf8 COLLATE utf8_general_ci NULL COMMENT '返回示例数据',
...
) ENGINE = MyISAM AUTO_INCREMENT = 202 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '接口' ROW_FORMAT = Dynamic;
...
CREATE TABLE `ya_relation`  (
  ...
  INDEX `project_id`(`project_id`) USING BTREE
) ENGINE = MyISAM AUTO_INCREMENT = 34 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '项目用户关联' ROW_FORMAT = Dynamic;
...
CREATE TABLE `ya_user`  (
  ...
  INDEX `user_id`(`user_id`) USING BTREE
) ENGINE = MyISAM AUTO_INCREMENT = 1015 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户' ROW_FORMAT = Dynamic;
